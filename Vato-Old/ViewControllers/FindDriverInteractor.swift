...
//        let p = Configs.api
        let p = "https://api-dev.vato.vn/api/user/check_info"
        let networkRouter = VatoAPIRouter.customPath(authToken:"", path: p, header: nil, params: params, useFullPath: true)
            ...
            .bind { [weak self](result) in
                ...
                case .success(let d):
                    if let data = d.data {
                        ...
                        self?.router?.goToBlock(driver: d)
                    }
                case .failure(let e):
