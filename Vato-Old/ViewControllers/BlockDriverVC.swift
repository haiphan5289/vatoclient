...
//    private lazy var noItemView = NoItemView(imageName: "driver-icon", message: Text.blockDriverMess.localizedText, on: tableView)
    private var noItemView: NoItemView?
}
...
private extension BlockDriverVC {
    ...
    private func visualize() {
        ...
        let customLayout: (NoItemView) -> () = { v in
            v.lblMessage?.snp.makeConstraints { (make) in
                make.left.right.equalTo(self.view).inset(10)
            }
        }
        
        noItemView = NoItemView(imageName: "driver-icon", message: Text.blockDriverMess.localizedText, on: tableView, customLayout: customLayout)
    }
    ...
    private func setUpRX() {
        ...
        listener?.blackListObser.bind(onNext: weakify { (list, wSelf) in
            ...
            list.count == 0 ? wSelf.noItemView?.attach() : wSelf.noItemView?.detach()
        ...
        tableView.rx.itemSelected.bind { (idx) in
            guard let list = self.listDriver else { return }
            var d = list[idx.row]
            d.type = .remove
            self.listener?.goToDetailDriver(driver: d)
        }.disposed(by: disposeBag)
    }
}

final class BlockDriverCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        iconImageView = UIImageView.create {
    ...
    func displayLayout(driver: BlockDriverInfo) {
        iconImageView?.setImage(from: driver.avatarUrl, placeholder: UIImage(named: "ic_default_avatar"), size: CGSize(width: 44, height: 44))
