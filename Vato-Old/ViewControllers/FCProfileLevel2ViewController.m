    ...
    RAC(self.btnContinue, enabled) = [RACSignal combineLatest:@[self.tfName.rac_textSignal,
                                                       ...
                                                       reduce:^(NSString* name,
                                                           ...
                                                           if (name.length == 0 ||
                                                               ...
                                                               if (name.length == 0)
                                                                   self.lblError.text = localizedFor(@"Thiếu họ tên (phải trùng với CMND)");
                                                               else if (email.length > 0 && ![self validEmail:email])
                                                                   self.lblError.text = localizedFor(@"Email không đúng");
                                                               else if (birthday.length == 0)
                                                                   self.lblError.text = localizedFor(@"Thiếu ngày tháng năm sinh");
                                                               else if (cmnd.length == 0)
                                                                   self.lblError.text = localizedFor(@"Thiếu CMND");
                                                               else if (cmndDate.length == 0)
                                                                   self.lblError.text = localizedFor(@"Thiếu ngày cấp CMND");
                                                               else if (cmndPlace.length == 0)
                                                                   self.lblError.text = localizedFor(@"Thiếu nơi cấp CMND");
                                                               return @(NO);
                                                           }
...
- (IBAction)birthdayTouch:(id)sender {
    ...
    IQActionSheetPickerView *picker = [[IQActionSheetPickerView alloc] initWithTitle:localizedFor(@"Chọn ngày") delegate:self];
