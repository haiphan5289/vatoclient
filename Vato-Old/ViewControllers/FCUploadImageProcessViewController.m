...
- (void)viewDidLoad {
    ...
    if (self.imageType == FONT_CMND) {
        self.lblTitle.text = localizedFor(@"Mặt trước CMND / Hộ Chiếu");
    }
    else if (self.imageType == BACK_CMND) {
        self.lblTitle.text = localizedFor(@"Mặt sau CMND / Hộ Chiếu");
    }
    else if (self.imageType == AVATAR) {
        self.lblTitle.text = localizedFor(@"Hình đại diện");
    }
}
