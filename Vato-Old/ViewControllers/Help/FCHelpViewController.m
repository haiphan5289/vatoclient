...
- (void)viewDidLoad {
    ...
    [[FirebaseHelper shareInstance] getAppConfigure:^(FCAppConfigure* appconfigure) {
        _listMenus = appconfigure.client_help_menus;
        [self.tableView reloadData];
    }];
}
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    self.navigationItem.title = localizedFor(@"Trợ giúp");
}
...
- (void) mailComposeController:(MFMailComposeViewController *)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError *)error {
    if (result == MFMailComposeResultSent) {
        [[FCNotifyBannerView banner] show:nil
                                 ...
                                 autoHide:YES
                                  message:localizedFor(@"VATO xin chân thành cảm ơn những đóng góp của bạn.")
                               closeClick:nil
                              bannerClick:nil];
    }
