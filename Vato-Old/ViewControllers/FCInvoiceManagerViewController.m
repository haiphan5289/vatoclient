...
@property (assign, nonatomic) BOOL shouldCheckData;
...
- (instancetype) initView {
    ...
    self.navigationItem.title = localizedFor(@"Lịch sử giao dịch");
    return self;
}
...
- (void) reloadData {
    ...
    if (self.listInvoice.count == 0) {
        ...
        [view show:self.view
             image:[UIImage imageNamed:@"no-trans"]
             title:nil
           message:localizedFor(@"Bạn chưa thực hiện giao dịch nào.")];
...
- (void) checkingData {
    if (self.listInvoice.count == 0) {
        ...
        view.messColor = [UIColor darkGrayColor];
        [view show:self.view
             image:[UIImage imageNamed:@"no-invoice"]
             title:nil
           message:localizedFor(@"Hiện bạn chưa có giao dịch nào được thực hiện trên hệ thống.")];
        [self.tableView setScrollEnabled:NO];
    }
}
