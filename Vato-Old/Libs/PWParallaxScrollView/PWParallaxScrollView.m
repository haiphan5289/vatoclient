...
- (void)initControl
{
    ...
    self.foregroundScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 20)];
    ...
    rect.origin.y += 10;
    self.backgroundScrollView = [[UIScrollView alloc] initWithFrame:rect];
...
- (void)reloadData
{
    ...
    for (NSInteger i = 0; i < _numberOfItems; i++) {
        UIView* bg = [self backgroundViewAtIndex:i];
        [self.backgroundScrollView addSubview:bg];
    }
}
...
- (UIView*)foregroundViewAtIndex:(NSInteger)index
{
    ...
    newFrame.origin.x += index * (CGRectGetWidth(self.frame))/2;
...
- (UIView*) getViewAtIndex: (NSInteger) index {
    if (self.backgroundScrollView.subviews.count > index + 1) {
        return [self.backgroundScrollView.subviews objectAtIndex:index + 1]; // index = 0 is contentview
    }
    return nil;
}
...
- (UIView*)backgroundViewAtIndex:(NSInteger)index
{
    ...
    NSInteger num_items = [(ServiceCollectionViewCell*) view listCartype].count;
    int w = num_items == 2 ? 206 : 310;
    [view setFrame:CGRectMake(index * CGRectGetWidth(self.frame) + ([UIScreen mainScreen].bounds.size.width - w)/2, 0, w, CGRectGetHeight(self.frame)*0.9)];
    [view setTag:index];
    DLog(@"Background view (%f, %f, %f, %f) => screenW: %f", view.frame.origin.x, view.frame.origin.y, view.frame.size.width, view.frame.size.height, [UIScreen mainScreen].bounds.size.width)
    return view;
}
...
//    [[_backgroundScrollView subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
//
//    NSInteger count = self.backgroundScrollView.subviews.count;
