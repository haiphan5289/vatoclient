...
import SnapKit
 ...
 @objcMembers final class NoItemView: UIView {
    ...
    private func common() {
        ...
        } >>> self >>>
        {
            $0.snp.makeConstraints({ (make) in
                ...
                make.centerY.equalTo(self.snp.centerY).offset(-edge.top)
            })
        }
        ...
        } >>> container >>>
        {
            $0.snp.makeConstraints({ (make) in
                ...
                make.width.lessThanOrEqualTo(self.snp.width).offset(-100).priority(.high)
