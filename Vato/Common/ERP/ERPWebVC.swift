...
import AVFoundation
import WebKit
final class ERPWebVC: UIViewController {
    ...
    convenience init(with url: URL?, title: String?, type: WebVCType) {
...
extension ERPWebVC {
    ...
    static func loadWebCustom(on controller: UIViewController?, url: URL?, title: String?, type: WebVCType) {
        ...
        let webVC = self.init(with: url, title: title, type: type)
...
private extension ERPWebVC {
    ...
    private func visualize() {
        ...
        btnClose.setImage(image?.withRenderingMode(.alwaysTemplate) ,for: .normal)
        let imgDismiss = UIImage(named: "ic_close_vato")
        btnDismiss.setImage(imgDismiss ,for: .normal)
    ...
    private func updateToken() {
        FirebaseTokenHelper.instance.eToken.filterNil().skip(1).delay(.seconds(1), scheduler: MainScheduler.asyncInstance).bind(onNext: weakify({ (token, wSelf) in
            wSelf.webView.evaluateJavaScript("window.action.refreshMobileToken('\(token)')") { (r, e) in
                #if DEBUG
                   if let e = e {
...
extension ERPWebVC: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        // Check
        #if DEBUG
        print("ERP: \(navigationAction)!!!!!")
        #endif
        if let url = navigationAction.request.url {
            let type = FileMediaType.identifyMedia(from: navigationAction.request.url)
            if type != .unknown {
                switch type {
                case .image:
                    let image = AXPhoto(url: url)
