...
enum AddNewDestinationType {
    ...
    var title: String {
        switch self {
        case .new:
            return Text.inTripAddDestination.localizedText
        case .edit:
            return "Đổi điểm đến"
        }
    }
    
    var titleButton: (cancel: String, accept: String) {
        switch self {
        case .new:
            return (Text.inTripCancelRequestDestination.localizedText, Text.inTripAddDestination.localizedText)
        case .edit:
            return (Text.inTripCancelRequestDestination.localizedText, "Đổi điểm đến")
        }
    }
}
...
protocol AddDestinationConfirmDependency: Dependency {
    // todo: Declare the set of dependencies required by this RIBe, but cannot be created by this RIB.
}
...
protocol AddDestinationConfirmBuildable: Buildable {
    func build(withListener listener: AddDestinationConfirmListener, type: AddNewDestinationType, tripId: String) -> AddDestinationConfirmRouting
}
...
final class AddDestinationConfirmBuilder: Builder<AddDestinationConfirmDependency>, AddDestinationConfirmBuildable {
               ...
               tripId: String) -> AddDestinationConfirmRouting
    {
        ...
        let changeConfirmBuilder = ChangeDestinationConfirmBuilder(dependency: component)
        return AddDestinationConfirmRouter(interactor: interactor, viewController: component.AddDestinationConfirmVC, changeDestinationConfirmBuildable: changeConfirmBuilder)
    }
}
