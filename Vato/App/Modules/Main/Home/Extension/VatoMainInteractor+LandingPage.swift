//  File name   : VatoMainInteractor+LandingPage.swift
//
//  Author      : Dung Vu
//  Created date: 5/28/20
//  Version     : 1.00
//  --------------------------------------------------------------
...
extension VatoMainInteractor {
    func refreshHomeLanding(ignoreCache: Bool = true) {
        DispatchQueue.ignoreCache = ignoreCache
        currentRequestPage = -1
        paging = .default
        mListSections = nil
        requestNextHomeSections()
        requestBannerHomeLanding()
    }
    
    func requestNextHomeSections() {
        let first = self.paging.first
        guard let next = self.paging.next else {
            return
        }
        
        guard currentRequestPage != next.page else {
            return
        }
        
        currentRequestPage = next.page
        guard first || !mLoadingRequest else {
            return
        }
        mLoadingRequest = true
        diposeLoadLandingPage?.dispose()
        diposeLoadLandingPage = requestSectionsHomeLanding(page: next.page)
            ...
            switch event {
            case .next(let items):
                let items = items.orNil([]).filter(\.status)
                if first {
                    wSelf.mCachedItems = items
                    wSelf.mListSections = .reload(items: items)
                } else {
                    wSelf.mListSections = .update(items: items)
                }
                wSelf.paging = PagingHome(page: next.page, canRequest: items.count > 0, size: next.size)
            case .error(let e):
                #if DEBUG
    ...
    private func requestSectionsHomeLanding(page: Int) -> Observable<[VatoHomeLandingItemSection]?> {
        let ignoreCache = DispatchQueue.ignoreCache
        return VatoLocationManager.shared.geoHash().flatMap { [weak self] (hash) -> Observable<[VatoHomeLandingItemSection]?> in
            guard let wSelf = self else { return Observable.empty() }
            let hash = hash
            var params: JSON = ["page": page]
            params["geohash"] = hash
            let router = VatoAPIRouter.customPath(authToken: "", path: "landing-page/home/sections", header: nil, params: params, useFullPath: false)
            return wSelf.network.request(using: router, decodeTo: OptionalMessageDTO<[VatoHomeLandingItemSection]>.self, ignoreCache: ignoreCache).map { try $0.get().data }
        }
    }
    
    private func requestBannerHomeLanding() {
        let ignoreCache = DispatchQueue.ignoreCache
        let request = VatoLocationManager.shared
            ...
            params["geohash"] = hash
            let router = VatoAPIRouter.customPath(authToken: "", path: "landing-page/home/sections/items", header: nil, params: params, useFullPath: false)
