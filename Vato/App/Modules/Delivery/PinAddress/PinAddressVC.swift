...
final class PinAddressVC: UIViewController, PinAddressPresentable, PinAddressViewControllable {
    ...
    private var userChange: Bool = false
    ...
    private var tapLocation: Bool = false
...
//            self?.userChange = true
            self?.tapLocation = true
            let coord = VatoLocationManager.shared.location?.coordinate ?? MapInteractor.Config.defaultMarker.address.coordinate
//            self?.listener?.lookupAddress(for: coord)
        ...
        self.listener?.placeDetailObservable.subscribe(onNext: weakify({ (model, wSelf) in
            if let model = model {
                wSelf.userChange = false
                wSelf.titleLabel.text = model.subLocality
                wSelf.chooseAddressBtn.isUserInteractionEnabled = true
                wSelf.chooseAddressBtn.alpha = 1
            } else {
                wSelf.titleLabel.text = Text.search.localizedText + "..."
                wSelf.chooseAddressBtn.isUserInteractionEnabled = false
                wSelf.chooseAddressBtn.alpha = 0.5
            }
        })).disposed(by: disposeBag)
    }
...
extension PinAddressVC: GMSMapViewDelegate {
    ...
    func mapView(_ mapView: GMSMapView, idleAt position: GMSCameraPosition) {
        guard userChange || tapLocation else { return }
        tapLocation = false
