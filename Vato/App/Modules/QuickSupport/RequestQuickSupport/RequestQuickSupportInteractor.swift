...
final class RequestQuickSupportInteractor: PresentableInteractor<RequestQuickSupportPresentable>, ActivityTrackingProgressProtocol {
    ...
    init(presenter: RequestQuickSupportPresentable,
         requestModel: QuickSupportRequest, defaultContent: String?) {
...
extension RequestQuickSupportInteractor: RequestQuickSupportPresentableListener {
    ...
    func submit() {
        ...
        UploadFirebaseStorage
            .uploadFireStoreMutiple(images: self.images, folderName: "supports")
            ...
            .subscribe(onNext: { [weak self] (urls) in
                ...
                self?.requestAPI()
                    ...
                    self?.presenter.showError(eror: error)
            }).disposeOnDeactivate(interactor: self)
    }
    
    func requestAPI() {
        guard let user = UserManager.instance.getCurrentUser() else { return }
        let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? ""
        let coor = UserManager.instance.getCurrentLocation()
