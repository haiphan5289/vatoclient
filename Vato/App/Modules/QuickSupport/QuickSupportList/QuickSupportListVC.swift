...
final class QuickSupportListVC: UIViewController, QuickSupportListPresentable, QuickSupportListViewControllable, DisposableProtocol, PagingListRequestDataProtocol, LoadingAnimateProtocol  {
...
struct QuickSupportListResponse: InitializeValueProtocol, ResponsePagingProtocol {
...
extension QuickSupportListResponse {
    init?(use value: Any?) {
        guard let items = value as? [QuickSupportModel] else {
            return nil
        }
        var new = QuickSupportListResponse(values: items, next: false)
        new.values = items
        self = new
    }
}
...
private extension QuickSupportListVC {
    ...
    private func visualize() {
        ...
        self.title = Text.requestedLists.localizedText
        ...
        let requestSupportButton = UIButton.create { (button) in
            ...
            button.setTitle(Text.sendSupport.localizedText, for: .normal)
