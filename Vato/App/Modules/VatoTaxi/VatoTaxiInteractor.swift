//  File name   : VatoTaxiInteractor.swift
//
//  Author      : Vato
...
protocol VatoTaxiRouting: Routing, VatoTaxiIntripProtocol {
...
protocol VatoTaxiListener: class {
...
final class VatoTaxiInteractor: Interactor, VatoTaxiInteractable, UsePromotionProtocol, BookingDependencyProtocol, ActivityTrackingProgressProtocol {
    weak var router: VatoTaxiRouting?
    weak var listener: VatoTaxiListener?
    let component: BookingConfirmTaxiType
    ...
    init(component: VatoTaxiComponent) {
    ...
    private func setupRX() {
        ...
        booking.subscribe(onNext: {[weak self] (booking) in
    ...
    func findSelectDefault(from values: [FareDisplay]) {
        ...
        var nextService = Car(id: current.setting.service, choose: true, name: current.setting.name ?? "", description: "")
        if let customSelect = self.currentBook?.defaultSelect {
            /// Need to check all
            if let n = values.first(where: { $0.setting.serviceType == customSelect.service }) {
                nextService = Car(id: n.setting.service, choose: true, name: n.setting.name ?? "", description: "")
            }
        }
        let select = ServiceChooseGroup(idService: nextService.id, service: nextService, fare: current)
        self.component.transportStream.update(select: select)
    }

    func detachMe() {
        // move to deinit to prevent miss case call cancel promotion
//        if let promotionModel = component.currentPromotion {
//            listener?.cancel(promotion: promotionModel)
//        }
        listener?.refeshLocation()
    ...
    private func prepareData() {
        ...
        booking.observeOn(MainScheduler.asyncInstance).bind { [weak self] booking in
    ...
    private func findRoute() {
        directionFares.map { $0.direction_info }.take(1).bind { [weak self](router) in
            guard let wSelf = self else { return }
            wSelf.subjectRoute.onNext(router.route)
            wSelf.router?.drawRoutes(using: router.overviewPolyline)
        }.disposeOnDeactivate(interactor: self)
    }
...
extension VatoTaxiInteractor: SafeTimerDelegate {
...
extension VatoTaxiInteractor {
    ...
    func applyServiceMore() {
        ...
        Observable.combineLatest(listAdditionalServicesObserable, priceStream, service).map({ (list, price, service) -> Double in
            let changableList = list.filter { $0.service! == service.service.id }
...
// MARK: InTripHandler
extension VatoTaxiInteractor: Weakifiable {
    func inTripMoveBack() {
