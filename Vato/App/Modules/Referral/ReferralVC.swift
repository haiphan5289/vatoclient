...
final class ReferralVC: UIViewController, ReferralPresentable, ReferralViewControllable {
    struct Config {
        static let title = Text.referralCode.localizedText
        ...
        static let nextTitle = Text.inviteFriend.localizedText
        static let copied = Text.copied.localizedText
    }
    ...
    private lazy var noItemView = NoItemView(imageName: "ic_referral_error", message: Text.thereWasAnError.localizedText, on: view)
    ...
    func showError(from error: Error) {
        ...
        noItemView.attach()
//        let action = AlertAction.init(style: .default, title: "OK", handler: {})
//        AlertVC.show(on: self, title: "Lỗi", message: "Hệ thống có lỗi vui lòng thử lại sau.", from: [action], orderType: .horizontal)
    }
}
...
private extension ReferralVC {
    ...
    private func setupRX() {
        ...
        self.listener?.referral.subscribe(onNext: { [weak self] in
            self?.response = $0
            self?.createView()
        }).disposed(by: disposeBag)
    }
