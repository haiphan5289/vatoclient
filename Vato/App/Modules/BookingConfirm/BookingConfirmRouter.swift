//  File name   : BookingConfirmRouter.swift
//
//  Author      : Vato
...
enum PromotionCheckType {
    case next
    case review
}

typealias BookingConfirmExpandListener = NoteListener & TransportServiceListener & TipListener & ConfirmBookingChangeMethodListener & ConfirmDetailListener & BookingConfirmPromotionListener & PromotionListener & PromotionDetailListener & BookingRequestListener & SwitchPaymentListener & ConfirmBookingServiceMoreListener & InTripListener & WalletListener
protocol BookingConfirmInteractable: Interactable, BookingConfirmExpandListener {
    var router: BookingConfirmRouting? { get set }
    var listener: BookingConfirmListener? { get set }
...
protocol BookingConfirmViewControllable: ViewControllable, ControllableProtocol {
...
final class BookingConfirmRouter: Router<BookingConfirmInteractable>, BookingConfirmRouting, RibsAccessControllableProtocol, LoadingAnimateProtocol, DisposableProtocol {
    ...
    init(interactor: BookingConfirmInteractable,
    ...
    private func reset() {
        ...
        mapView.padding = UIEdgeInsets(top: paddingTop + mapView.edgeSafe.top - 10, left: 16, bottom: 385, right: 40)
    }
...
extension BookingConfirmRouter {
    ...
    private func setupRX() {
        ...
        self.bookingConfirmView
            .eAction
            .debounce(RxTimeInterval.milliseconds(30), scheduler: MainScheduler.asyncInstance).bind { [weak self] in
