...
extension SetLocationInteractor: SetLocationInteractable {
    ...
    private func update(model: AddressProtocol) {
        updateAddress(model: model, update: weakify({ (new, wSelf) in
            wSelf.mLastestLocation = new
        }))
    }
}

// MARK: SetLocationPresentableListener's members
extension SetLocationInteractor: SetLocationPresentableListener, LocationRequestProtocol, Weakifiable {
    ...
    func setDefaultLocation() {
        $mLastestLocation.take(1).observeOn(MainScheduler.asyncInstance).bind {[weak self] (address) in
            guard let wSelf = self else { return }
            PlacesHistoryManager.instance.add(value: address, increase: false)
...
private extension SetLocationInteractor {
    ...
    private func getLatestLocation() {
        PlacesHistoryManager.instance.searchListLastest(isOrigin: true).take(1).bind {[weak self] (listLatest) in
            guard let wSelf = self else { return }
            
            wSelf.mLastestLocation = listLatest.first ?? MapInteractor.Config.defaultMarker.address
            
        }.disposeOnDeactivate(interactor: self)
        
        let displayName = UserManager.instance.info?.displayName ?? UserManager.instance.info?.fullName
