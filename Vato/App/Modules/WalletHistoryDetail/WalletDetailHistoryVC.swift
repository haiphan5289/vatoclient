...
private extension WalletDetailHistoryVC {
    ...
    private func visualize() {
        ...
        self.title = Text.transactionDetails.localizedText
    ...
    private func setup(by item: WalletItemDisplayProtocol) {
        ...
        source.append([WalletHistoryItemDisplay(title: Text.transactionId.localizedText, value: "\(item.id)", color: nil),
                       WalletHistoryItemDisplay(title: Text.date.localizedText, value: s, color: nil)])
        ...
        for s in source.enumerated() {
            ...
            case 0:
                for item in s.element {
                    let row = Row<WalletHistoryDetailTitleCell>.init(tag: item.title)
                    row.value = item
                    section <<< row
                }
            case 1:
                for item in s.element {
                    let row = Row<WalletHistoryDetailItemCell>.init(tag: item.title)
                    row.value = item
                    section <<< row
                }
            case 2:
                for item in s.element {
                    let row = Row<WalletHistoryDetailDescriptionCell>.init(tag: item.title)
                    row.value = item
                    section <<< row
                }
            default:
    ...
    private func setupRX() {
        ...
        self.listener?.source.subscribe(onNext: { [weak self] in
            self?.setup(by: $0)
        }).disposed(by: disposeBag)
    }
}
