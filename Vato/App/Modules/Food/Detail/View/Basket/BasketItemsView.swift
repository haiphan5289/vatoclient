...
final class BasketItemsTVC: UITableViewCell, UpdateDisplayProtocol {
    private lazy var productImageView: UIImageView = UIImageView(frame: .zero)
    ...
    private func visualize() {
        ...
        stackView >>> contentView >>> {
            ...
            $0.snp.makeConstraints({ (make) in
                make.edges.equalTo(UIEdgeInsets(top: 16, left: 76, bottom: 16, right: 16))
            })
        }
    ...
    func setupDisplay(item: BasketItemType?) {
        ...
        productImageView.setImage(from: product, placeholder: nil, size: CGSize(width: 48, height: 48))
...
final class BasketItemsView: UIView, Weakifiable, DraggableViewProtocol {
    ...
    struct Configs {
        ...
        static let payment = Text.continue.localizedText
        static let title = Text.basket.localizedText
    }
    ...
    private var mDeleteItem = PublishSubject<DisplayProduct>()
    var select: Observable<DisplayProduct> {
        return mSelect
    }
    
    var deleteItem: Observable<DisplayProduct> {
        return mDeleteItem
    }
...
private extension BasketItemsView {
    ...
    func setupRX() {
        ...
        state.skip(1).bind(onNext: weakify({ (state, wSelf) in
            switch state {
            case .none:
    ...
    func handler(cell: BasketItemsTVC, idx: Int) {
        ...
        cell.btnDelete.rx.tap.takeUntil(e).map { [weak self] in
            self?.source[safe: idx]?.product
        }.filterNil()
        .bind(onNext: weakify({ (item, wSelf) in
            wSelf.mDeleteItem.onNext(item)
        })).disposed(by: disposeBag)
    }
}
